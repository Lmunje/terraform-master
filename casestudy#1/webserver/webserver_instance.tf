...
resource "aws_security_group" "lionel_webservers"{
  tags = {
    Name = "${var.ENVIRONMENT}-lionel-webservers"
  }
...
resource "aws_launch_configuration" "launch_config_webserver" {
  ...
  security_groups = [aws_security_group.lionel_webservers.id]
...
resource "aws_autoscaling_group" "lionel_webserver" {
  name                      = "lionel_WebServers"
  max_size                  = 2
  min_size                  = 1
...
resource "aws_lb" "lionel-load-balancer" {
  name               = "${var.ENVIRONMENT}-lionel-lb"
  internal           = false
  load_balancer_type = "application"
  security_groups    = [aws_security_group.lionel_webservers_alb.id]
...
resource "aws_lb_listener" "webserver_listner" {
  load_balancer_arn = aws_lb.lionel-load-balancer.arn
  port              = "80"
  protocol          = "HTTP"
...
output "load_balancer_output" {
  value = aws_lb.lionel-load-balancer.dns_name
}
